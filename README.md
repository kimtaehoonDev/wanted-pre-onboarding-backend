...
- [x] 단위 테스트 작성
- [x] 통합 테스트 작성
- [x] docker compose를 이용해 애플리케이션 환경 구성
- [x] AWS에 애플리케이션 배포
- [x] 회원가입
- [x] 로그인
- [x] 게시글 CRUD & 페이징
...
1. 작업 디렉토리를 생성합니다.
2. 작업 디렉토리에 .env 파일을 생성합니다. 파일의 내용은 다음과 같습니다.
   ```
   ...
   ```
3. 작업 디렉토리에 docker-compose.yml 파일을 생성합니다.
   ```
   version: '3'
   services:
    web:
...
#### Post 엔티티에 writer_email이라는 컬럼을 역정규화했습니다.
   - 게시판이라면 페이징 조회가 자주 일어난다고 생각했습니다. <br>
매 페이징마다 작성자의 이메일을 보여주기 위해 조인 연산을 하는 건 성능상 문제가 될 수 있다 생각했습니다.<br>
최적화를 위해 Post 엔티티에 writer_email 컬럼을 역정규화했습니다.
